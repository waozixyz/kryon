...
const HeaderSize = 48 // Size of KRB v0.3 header
...
// KrbPropertyDefinition describes a property declared in a component's Properties block.
// This is how it's stored in the KRB Component Definition Table.
type KrbPropertyDefinition struct {
	NameIndex        uint8     // 0-based index into String Table for property name
	ValueTypeHint    ValueType // VAL_TYPE_* hint for expected data type
	DefaultValueSize uint8     // Size in bytes of DefaultValueData
	DefaultValueData []byte    // Raw byte data for the default value
}

// KrbComponentDefinition represents a single entry in the KRB Component Definition Table.
type KrbComponentDefinition struct {
	NameIndex           uint8                   // 0-based index into String Table for component's name
	PropertyDefCount    uint8                   // Number of property definitions
	PropertyDefinitions []KrbPropertyDefinition // Slice of property definitions
	// RootElementTemplate: This is more complex. It's a full Element Block.
	// We'll store its raw bytes for now, or parse it into existing Element structures.
	// For simplicity in this step, let's store the raw bytes of the template.
	// A more advanced reader might parse this into a separate Element tree.
	RootElementTemplateData []byte // Raw bytes of the Element Block for the template
}

// Add KrbComponentDefinitions to your Document struct
type Document struct {
    ...
    Styles              []Style
    Animations          []byte // Placeholder for now
    Strings             []string
    Resources           []Resource
    ChildRefs           [][]ChildRef
